...
## TASK #16 ROOT CAUSE — 2026-05-24 (RESOLVED)

Historically, `BDPT::setProperties` force-re-pinned `useBPT=true` (plus
the ReSTIR knobs) after parsing Python properties, silently overriding
debug configs like `useBPT: False` / `useNEE: False`. Combined with
`USE_NEE = (useNEE || useBPT) ? "1" : "0"`, forcing `useBPT=true`
also forced NEE on. So debug-decomposition runs were misleading: a
`bsdf` config actually ran with USE_BIDIRECTIONAL=1, USE_NEE=1.

**Resolved** by task #21 (strip-ReSTIR): the ReSTIR knobs no longer
exist on StaticParams, so there is nothing to re-pin. `setProperties`
now just calls `parseProperties` + `validateOptions` — debug configs
(`useNEE: False`, `useBPT: False`) take effect as authored.

The Arcade 2% / Sponza 2.4% over-shoot remains, and is the residual
parallel-vs-unified light-type-selection MIS difference (see
"Architectural note" above). The unified-`selectLightType` refactor
attempt was reverted because the BPT MIS weight in
`EvalDirectLightMIS` depends quadratically on `directPdfW` — making
it work requires splitting the pdf used for MIS (unscaled) from the
pdf used for integration-weight normalization (selection-scaled).
That's task #16 proper, still pending.
