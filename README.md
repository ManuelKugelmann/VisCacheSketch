...
The spatial grids of the hash map were visible in the thesis — screenshots show grid cells. What was an unmentioned implementation detail was the use of *spatial hashing* to map grid cells to memory — inspired by ODE's (Open Dynamics Engine) use of spatial hasing in broad pahse collision detection to sidestep the curse of dimensionality in naive grids - probably based on [Teschner et al. 2003]'s work. The thesis used it as an implementation detail but did not describe or frame it as a technique.
