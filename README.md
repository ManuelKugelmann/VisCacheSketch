...
The spatial grids were visible in the thesis (screenshots show grid cells). The grids used spatial hashing [Teschner et al. 2003] internally to map cells to memory. The practical exposure came through **ODE** (Open Dynamics Engine) [Smith 2001–2004], whose `dHashSpace` broad-phase collision system uses spatial hashing — encountered hands-on during the **Animal Race** 3D racing game (Jan–Oct 2005), a Universität Ulm student course project using ODE (hosted at [animalrace.bitcraft.org](http://animalrace.bitcraft.org/)), and reinforced during TA work on Keller's "Simulation Algorithms" lecture. Hashing itself was an unmentioned implementation detail, not described as a technique in the thesis.
...
| Animal Race (2005) | Uni Ulm student course project — 3D racing game using ODE; hands-on exposure to spatial hashing (hosted at [animalrace.bitcraft.org](http://animalrace.bitcraft.org/)) |
