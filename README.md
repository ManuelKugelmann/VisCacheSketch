...
|---|---|---|
| CV+VRRR with localized per-point cache | [Szécsi et al. 2003][r-szecsi] (CV, fixed RR), [Szirmay-Kalos et al. 2005][r-szirmay] (variance-driven RR, scene-global estimate), [Guo et al. 2020][r-guo] (NEE++, per-voxel-pair visibility), [Bokšanský & Meister 2025][r-boksansky] (neural per-point visibility) | Visibility caching predates 2006 ([Ward 1991][r-ward]). CV+RR was known ([Szécsi 2003][r-szecsi]). Variance-driven RR was known ([Szirmay-Kalos 2005][r-szirmay]). What 2006 combined: per-point cached predictions as CV estimation source + variance from cache quality driving RR — the localized cache is what makes CV+VRRR effective vs. a scene-global constant |
