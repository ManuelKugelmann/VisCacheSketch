...
**Why it was cut short.** The 2006 thesis was looking for a workable solution to the concrete problem of the day — rendering many lights, many shadow rays, and many paths in tractable time. There was no "substrate" lens or any view of where the field would converge two decades later; the goal was just a reuse scheme that worked. The bidirectional/backtracing part of the thesis already attempted reconnection-style sample reuse — inspired by Metropolis-style mutations combined with the imperfect reconnections of photon mapping. The sensor side was the natural starting point: reconnections there can be loose without introducing bias or wasting paths that miss the sensor entirely — the pixel filter already integrates over a 2D area, so a path landing anywhere within the filter contributes to that pixel's estimate. None of it was carried to a finished publication: funding pressure throughout the Diplomarbeit pulled time toward paid sidework, and afterwards a paid Computer-Vision thesis became the priority. The visibility-caching line was left at "promising sketch with empirical results" — the polish, broader scene validation, and journal write-up never happened. The 2026 work picks the same investigation back up two decades later.
...
flowchart TB
    ...
    %% (Not the cache substrate — the path-transformation machinery threading through ReSTIR PT.
    ...
    %% ============ Fringes: same substrate, parallel fields (non-rendering) ============
    NIES13(["2013 Nießner et al.<br/>Real-time 3D Reconstruction<br/>at Scale (voxel hashing, TSDF)"]):::fringe
    NVDB21(["2021 Museth<br/>NanoVDB<br/>(GPU sparse voxel substrate)"]):::fringe
    ...
    %% ---- Same-substrate links into the rendering convergence (no citation) ----
    INGP -. "same substrate: multi-res hash" .-> US
    MRH -. "same substrate: variance-adaptive hash" .-> US
    FVDB -. "same substrate: topology/value split" .-> US
...
- **Sparse voxel substrates**: NanoVDB [Museth 2021] → fVDB [Williams et al. 2024] adds explicit topology/value separation that is structurally similar to splitting our `WSCellPool` slot table from the cascade hash.
