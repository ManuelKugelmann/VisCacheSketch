...
|---|---|---|
| CV+VRRR with localized per-point cache | [Szécsi et al. 2003][r-szecsi] (CV, fixed RR), [Szirmay-Kalos et al. 2005][r-szirmay] (variance-driven RR, scene-global estimate), [Guo et al. 2020][r-guo] (NEE++, per-voxel-pair visibility) | Skipping predictable shadow rays predates 2006 ([Ward 1991][r-ward] — heuristic ordering, not a spatial cache). CV+RR was known ([Szécsi 2003][r-szecsi]). Variance-driven RR was known ([Szirmay-Kalos 2005][r-szirmay]). What 2006 combined: per-point spatial cache as CV estimation source + variance from cache quality driving RR — the localized cache is what makes CV+VRRR effective vs. a scene-global constant |
| Spatial hash map against curse of dimensionality | [Binder et al. 2018][r-binder] (path-space filtering), [Gautron 2020][r-gautron20]/[2021][r-gautron21] (AO), [SHaRC (Benyoub et al. 2024)][r-sharc] (world-space spatial hash, RTX SDK), [Guo et al. 2020][r-guo] (NEE++, dense D³×D³ matrix — hits the dimensionality wall) | 2006 used spatial hashing from [ODE][r-ode] for compact pairwise storage but did not describe it as a contribution; NEE++ later hit the dimensionality wall with its 16³×16³ matrix |
...
| Algorithm-agnostic pairwise cache | — | 2006 tested on instant radiosity; the cache operates on (point, point) → {0,1} regardless of algorithm |
| Visibility cache + cache-weighted light selection | [Popov et al. 2013][r-popov] (adaptive octree visibility cache, offline, <2% rays), [Bokšanský & Meister 2025][r-boksansky] (neural visibility cache + μ-weighted reservoir sampling for light selection) | Popov: octree-based visibility cache with adaptive resolution (vs. our hash-based with variance-gated LoD). Bokšanský: parallel rediscovery of the full pipeline — cached visibility *and* μ-weighted ReSTIR candidate selection (our Sec. 9.1); Instant-NGP backbone instead of spatial hash |
