...
def run_baseline_ReSTIRDI_R2dP2d_PoolOnly(step_name, frame_configs, scene_file,
    ...
    Tests whether dropping the 17 per-pixel PdfMipmap descents (the
    fresh K-RIS samples) recovers RTXDI's structural perf advantage.
    Our cell-pool with poolAddrMode=1 IS a presample tile — reads from
    it are cheap (atomic load + light rebuild), unlike fresh K-RIS
    which does a full mipmap descent per pixel per candidate.
    ...
    return _run_baseline_restir(
        step_name, frame_configs, scene_file,
        tag_prefix="ReSTIRDI_R2dP2d_PoolOnly",
        addr_mode_kwargs={"poolAddrMode": 1, "poolTileSize": poolTileSize},
        initialCandidates=0,                       # F0 — no fresh K-RIS
        cellPoolDrawK=24,                          # P24 — all pool reads
        wsCellPoolPrePass=True,                    # KEEP prepass to fill pool
        prePassEmissiveSampler="PdfMipmap",
        **kwargs2,
    )
...
def run_baseline_ReSTIRDI_R2dP2d_RTXDIBaseline(step_name, frame_configs, scene_file,
    ...
    **Param parity is the contract.** Where our quality trails RTXDI, the
    ...
    return _run_baseline_restir(
        ...
        # We match K_total=41 with: 24 pool (emissive, PdfMipmap) + 17
        # uniform-fresh main-pass-LightBVH. The 17 fresh route through
        # selectLightType uniform roulette (1/3 emissive, 1/3 env,
        # 1/3 inf in expectation) ≈ 5.67 per category — close enough
        # to RTXDI's 0/8/8/1 dedicated split modulo BRDF, and the
        # outer-NEE-style MIS already mixes BRDF into the candidate
        # distribution. F17 uniform stream is mathematically the
        # equivalent of RTXDI's category split at K_total=41 (per
