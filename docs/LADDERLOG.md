...
| TIMING | vanilla vs cache wall-clock on Sponza x{4, 16} (Falcor PROFILER) | **First real ms numbers.** Falcor PROFILER hook in run_variants AND run_baseline → gpu_tracepass_ms in CSV. Sponza canonical at stderr=0.10: x4 vanilla=20.6 ms / cache=15.87 ms → **23% wall-clock saved** (vs algorithmic 13% rays); x16 vanilla=18.05 / cache=16.98 → **6% saved** (vs algorithmic 27% rays). Wall-clock and rays_traced_pct DON'T track linearly — at low SPP cache savings exceed the rays-saving (cache lookup overhead < shadow-ray cost on Sponza); at high SPP cell-write atomic contention grows with frame count, eating into the rays-savings benefit. **For real-time / low-SPP use cases (single-frame, x4), cache savings track rays well; offline / many-frame regimes have an overhead floor.** Establishes the methodology for any future cost-comparison sweep. First-variant warmup confound (~30% slow on first variant per scene-load) documented in `run_variants` source — render the same variant twice for clean comparisons. | gpu_tracepass_ms canonical telemetry; methodology for paper §13 results table; pitch headline "23% wall-clock saved at x4 on Sponza, vanilla-quality match" |
