...
| BISTRO_CT | 4-corner (ct,vt) on BistroExt + BistroInt | **Sponza framework does NOT generalize.** BistroExt x16 + BistroInt x4/x16 art5 **bit-identical** across all 4 corners (29.93 / 42.87). Bistro saturation is **structural** (bias-locked cells, not premature all-same trust). Penumbra-class scenes (Sponza) want patient trust gates; geometry-bias-locked scenes (Bistro) need active de-trust (accelDecayDisagreeThresh). One universal carry doesn't exist; per-scene-class config required | scene-classifier needed; Bistro needs decay/accel-decay, not ct/vt |
