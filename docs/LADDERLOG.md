...
| TIMING_HONEST | single-Mogwai vanilla-vs-cache, identical render loops back-to-back | **The honest wall-clock story is much weaker than TIMING claimed.** Bypasses run_baseline / run_variants harnesses entirely; both vanilla and cache use the same minimal `measure_one(build_fn)` loop with N_WARMUP=4 + RENDER_FRAMES=8 in a single Mogwai invocation. Sponza canonical (stderr=0.10): **b=0 vanilla=29.64 / cache=28.57 → +3.6% saved**, **b=4 vanilla=27.55 / cache=30.65 → −11.3%** (cache slightly slower at multibounce). The earlier TIMING-claimed +58% wall-clock saved on Sponza was inflated by cross-run variance (TIMING vanilla=44 ms vs TIMING_HONEST vanilla=29 ms for the same nominal workload — different Mogwai sessions land in different GPU clock/thermal states). **Reframing**: the cache's algorithmic story (13–94% rays saved across the matrix) is robust; the **wall-clock story is weaker** — current implementation pays per-pixel hash-lookup overhead that's roughly comparable to the saved-ray cost on Sponza single-bounce, and exceeds it at multibounce. This is the most reliable wall-clock data we have. | corrected pitch headline: "algorithm saves 13–94% rays at vanilla quality; current GPU implementation delivers ~3-4% wall-clock on Sponza single-bounce, with multibounce wall-clock requiring further GPU-engineering work — natural NVIDIA collaboration vector" |
