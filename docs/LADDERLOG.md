...
| SPONZA_WILSON | Wilson 95%/99% CI gate sweep on Sponza x{4,16}, ε ∈ {0.005..0.40} | **Wilson gate works but is SPP-asymmetric.** x4 (cold cells, low N): bit-identical across ε ∈ {0.005..0.40} — Wilson's wide CI refuses trust at low N regardless of ε margin; result matches vt=0.001 baseline (art5=18.04 vs vt=0.10's 17.53). x16 (mature cells): ε ∈ {0.005..0.20} also bit-identical to vt=0.001 (art5=15.21). **ε=0.40 at x16** is the only point that differentiates: art5=15.28 (+0.07pp), but **relmse 0.72 → 0.29 (2.5× better)**, PSNR +0.12 dB, rays −0.4pp. Wilson auto-collapses to "strict" at low SPP, exposes the looser trust regime at high SPP via large ε. **Not a strict improvement over per-SPP-tuned vt** — Wilson at any tested ε matches vt=0.001 closely (vt=0.001 is itself near-optimal at x16 except for relmse). The relmse improvement at ε=0.40 x16 is interesting and reframes the metric-anti-correlation finding: Wilson lets us pick "trust the mature cells more aggressively" at ε≥0.40 with controlled SPP-scaling, whereas raising vt blanket-applies the trust at all SPPs. | x16 only: ε=0.40 candidate carry for PSNR/relmse-priority workloads. x4: no Wilson advantage. SPP-stability is **not** delivered — Wilson is a strict-vt-equivalent within tested ε. |
