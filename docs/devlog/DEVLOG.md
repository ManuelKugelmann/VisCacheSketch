...
**Static-scene multilevel ladder (steps 11–17):**
...
**Net carry after step 17:** `pos_norm__pos__qa012__bayer4x4_cell4x4_ct16_vt003_pm010`, validated across 5 scenes. The vt=0.03 boundary is the cleanest result so far — it's the lowest variance threshold the cache can sustain without trusting un-converged cells, and the same boundary holds on Cornell, Sponza, and Bistro.

**Open work — the bias floor:** BistroInterior blob is 130–156 across all SPP/vt — and **x16 blob is HIGHER than x4** (false-trust regime persists even at the optimum vt). The cache makes BistroInterior worse with more samples, not better. Sponza x16 blob 89+, BistroExterior x16 blob 67. Trust-gate tightening cannot fix this — vt=0.03 is the floor below which Sponza/Bistro break, but residual blob is huge. Candidate next experiments: (a) higher ct revisit ({16, 64, 128, 256}) — ct=128 brought Sponza blob to 3.4 in the broken-cascade era; could it help in the new regime?; (b) per-level ct scaling; (c) addressing/quant axis revisit on Bistro specifically.
