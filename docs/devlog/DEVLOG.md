...
**Static-scene multilevel ladder (steps 11–16):**
...
**Net carry after step 16:** `pos_norm__pos__qa012__bayer4x4_cell4x4_ct16_vt003_pm010`. Steps 13/14/15 documented dead-ends but step 16 broke through by widening the vt axis below the original step-06 value. Information about the parameter space that wasn't visible from earlier broken-cascade ladders.

**Open work:** Sponza x16 blob is still stuck around 29 regardless of trust gate — that's the **bias floor**, intrinsic to Bernoulli μ on bias-dominated scenes. Trust-gate tightening doesn't fix it (this step confirms). Candidate next experiments: (a) even tighter vt sweep (0.005 / 0.01 / 0.03) — vt=0.03 was the *lowest* of the step-16 grid, optimum may be further down; (b) per-level ct scaling (ct tradeoff is opposite on Cornell 1PL vs Bistro); (c) bounce-test stage with maxBounces > 0.
