...
- **RTXDI BoilingFilter port** — implemented as a frame-start compute pass (`Source/RenderPasses/VisCache/WSReservoirBoilingFilter.cs.slang`) structurally identical to `VisCacheDecay.cs.slang` (per-frame maintenance on a persistent buffer). RTXDI algorithm faithfully ported (16×16 thread groups, wave reduction → group mean, threshold = `10/strength - 9`). Build clean; smoke clean; dispatch gate verified firing via `logInfo` (`WSRes=1 PxRes=1 BF=1 mpPx=1 mpPass=1 dim=512x512` every subframe). **Empirical result: shader-side writes do NOT effectively reach `gWSPixelReservoirs`** even when stripped to a minimal "unconditional clear" shader without wave intrinsics or groupshared. Diagnostic ladder: (a) host-side `clearUAV()` on the same buffer at the same dispatch site → Sponza_x4 6.49 → 6.39 (−0.10pp, real effect); (b) full algorithm with wave/groupshared → Sponza_x4 = 6.49 (no effect); (c) minimal unconditional-clear shader (no wave/groupshared/`#pragma once`) → Sponza_x4 = 6.47 (within noise). All three at strength=0.2; sanity check at strength=1.0 also shows 6.49. Conclusion: buffer mutation works in principle (host clear) and the dispatch fires (logInfo), but compute-shader writes don't make it through. Suspected slang-reflection / Falcor-binding mismatch on the locally-declared `RWStructuredBuffer<WSReservoir>` (the working `gVHFTable` in `VisCacheDecay` is *imported* from `VisCache.slang`, not redeclared). Pinpointing requires GPU profiler / PIX trace; the algorithm shape is committed as the RTXDI-equivalent on the table. Quality wouldn't change at canonical config either way — the existing Conv B reader-pdf + V-test pipeline is already firefly-free, so the filter has no work to do.
