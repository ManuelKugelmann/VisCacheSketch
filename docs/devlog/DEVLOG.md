...
- **RTXDI BoilingFilter port** — implemented as a frame-start compute pass (`Source/RenderPasses/VisCache/WSReservoirBoilingFilter.cs.slang`) structurally identical to `VisCacheDecay.cs.slang` (per-frame maintenance on a persistent buffer). RTXDI algorithm faithfully ported (16×16 thread groups, wave reduction → group mean, threshold = `10/strength - 9`). Build clean; smoke clean; dispatch gate verified firing via `logInfo` (`WSRes=1 PxRes=1 BF=1 mpPx=1 mpPass=1 dim=512x512` every subframe). **Empirical result on Sponza_x4: 6.49 → 6.49 at strength=0.2, 6.49 at strength=1.0 (most aggressive).** Diagnostic test bypassing shader with host-side `clearUAV()` on the same buffer DOES move the metric (6.49 → 6.39, −0.10pp), confirming the buffer mutation path itself works — but my compute-shader writes don't appear to reach the buffer (slang reflection / binding / barrier mismatch we couldn't pinpoint without deeper debugging tools). The implementation is committed as the RTXDI-equivalent shape on the table; activating its effect would require either resolving the shader-dispatch issue or routing through a known-good RenderGraph edge.
