...
- **Slang pinned to 2024.14.6** in `Falcor/dependencies.xml` (NOT Falcor 8's default 2024.1.34). Falcor itself is 8.0 + 2 post-release cherry-picks (already applied). When debugging shader compile / reflection / setVars errors, or before bumping slang or Falcor, read [`docs/SLANG_AND_FALCOR_NOTES.md`](docs/SLANG_AND_FALCOR_NOTES.md) — it catalogues every Falcor 7→8 API rename, every Slang strictness change we hit, and tested-vs-broken Slang version table.
...
- **If stuck on a problem, research the documentation and web for hints.** Before iterating blindly on a Slang / Falcor / GPU bug, check the relevant upstream docs ([Slang user guide](http://shader-slang.org/slang/user-guide/), [Slang release notes](https://github.com/shader-slang/slang/releases), [Falcor 8.0 release notes](https://github.com/NVIDIAGameWorks/Falcor/releases/tag/8.0), our own [`docs/SLANG_AND_FALCOR_NOTES.md`](docs/SLANG_AND_FALCOR_NOTES.md) and [`docs/PORTING.md`](docs/PORTING.md)). WebFetch / WebSearch for the exact error message — Falcor and Slang error messages are often Google-able to a fix or a Slang issue. Update the project doc with what you learn so the next session doesn't have to re-research.
