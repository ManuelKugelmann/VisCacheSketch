...
The core estimator, which we call *prediction-with-correction* following the naming in [Kugelmann 2006] (shorthand: CV+VRRR — Control Variate + Variance-driven Russian Roulette Residual), is a control-variate technique with variance-driven Russian roulette on the residual. The idea of reducing RR variance by returning a non-zero estimate on termination was proposed by Szécsi, Szirmay-Kalos and Kelemen [2003]; the broader "go with the winners" strategy [Szirmay-Kalos et al. 2005] extended this into a unified splitting/RR framework with variance-based child-ray counts. [Kugelmann 2006] developed prediction-with-correction independently for cached illumination predictions (thesis Sec. 3.4, "predictions with correction at random"), with variance-driven RR survival probability as adaptive sampling (Sec. 3.4.1). The idea of using variance — not absolute light — to drive sampling rate was inspired by hints in Keller's lectures at Universität Ulm. The technique is classical; we do not claim it as new. We apply it to a multilevel cache where the same variance signal now also drives spatial resolution (Sec. 5), a coupling that was impossible at fixed resolution.
